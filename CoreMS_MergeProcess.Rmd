...
2) Install tidyverse ("install_packages("tidyverse")")
3) Install devtools ("install_pakcages("devtools")")
4) Install ftmsRanalysis ("devtools::install_github("EMSL-Computing/ftmsRanalysis")")

## Load packages
This section loads all of the required R packages for this pipeline and allows you to tweak the import settings. You can specify your path to the files of interest and the type of thresholding you want to use below.
...
# lambda functions (included in the repo - point to where you stored the 
# R function) Provided with permission: https://github.com/hyunseobsong/lambda
...
This section is handling the hand off from the CoreMS outputs, while setting everything up to run smoothly through ftmsRanalysis. Currently, this is loading in .csv files that were generated using CoreMS above. This will navigate to the folder that you indicated above and load in every single .corems.csv; there is no mechanism to control for diverse datasets.
